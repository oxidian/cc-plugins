---
allowed-tools: Bash(uv run python plugins/oxgl/scripts/wait_for_ai_review.py:*), Bash(glab mr view:*), Bash(glab mr checkout:*), Bash(glab api:*), Bash(git remote get-url origin)
...
3. **Wait for AI review**: Run `uv run python plugins/oxgl/scripts/wait_for_ai_review.py <MR_IID>` (use 45 minute timeout)
...
5. **Analyze and respond**:
   - If there are findings: investigate the codebase to understand the issue, then immediately write a concrete implementation plan to fix them. Do NOT ask the user whether they want to fix the issues - assume they do. Write the full plan directly.
